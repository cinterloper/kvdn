...
//import net.iowntheinter.kvdn.kvsvc.kvsvcVertxProxyHandler;
...
String syncGetMethod(svc) {
    CompletableFuture future = new VertxCompletableFuture(v)

//I want to write the following:

    svc.get("straddr", "key", new JsonObject(), { AsyncResult<String> ar ->
        if (ar.succeeded())
...
String syncSetMethod(svc) {
    CompletableFuture future = new VertxCompletableFuture(v)

//I want to write the following:

    svc.set("straddr", "key", "value", new JsonObject(), { AsyncResult<String> ar ->
        if (ar.succeeded())
            future.complete(ar.result())
        else
            future.completeExceptionally(ar.cause())
