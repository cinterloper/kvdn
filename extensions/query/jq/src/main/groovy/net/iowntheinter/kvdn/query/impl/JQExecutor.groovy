...
// First of all, you have to prepare a Scope which s a container of built-in/user-defined functions and variables.
        ...
        rootScope.addFunction("repeat", 1, new Function() {
            @Override
            List<JsonNode> apply(Scope scope, List<JsonQuery> args, JsonNode input) throws JsonQueryException {
                final List<JsonNode> out = new ArrayList<>()
                for (final JsonNode times : args.get(0).apply(scope, input))
                    out.add(new TextNode(Strings.repeat(input.asText(), times.asInt())))
                return out
            }
        })
        ...
        JsonNode input = MAPPER.readTree("{\"ids\":\"12,15,23\",\"name\":\"jackson\",\"timestamp\":1418785331123}")
        ...
        final List<JsonNode> out = q.apply(childScope, input)
